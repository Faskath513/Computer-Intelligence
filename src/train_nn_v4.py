"""5th Model: Neural Network on V4 features (Keras MLP)"""
...
X_tr, X_val, y_tr, y_val = train_test_split(X_proc, y_enc, test_size=0.2, random_state=42, stratify=y_enc)

print(f"NN input dim: {X_tr.shape[1]}, classes: {n_classes}")

# StandardScaler for NN (tree models don't need it, but NN does)
from sklearn.preprocessing import StandardScaler
scaler = StandardScaler()
X_tr_s = scaler.fit_transform(X_tr)
X_val_s = scaler.transform(X_val)
X_all_s = scaler.transform(X_proc)
...
early_stop = tf.keras.callbacks.EarlyStopping(patience=8, restore_best_weights=True, monitor="val_loss")
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(factor=0.5, patience=3, min_lr=1e-6)

print("Training Neural Network (V4 features)...")
t0 = time.time()
history = nn.fit(
    X_tr_s, y_tr,
...
preds_val = np.argmax(nn.predict(X_val_s, verbose=0), axis=1)
ba = balanced_accuracy_score(y_val, preds_val)
print(f"NN V4 Balanced Accuracy: {ba:.4f}")

# Save model and generate submission
...
sub.to_csv(os.path.join(SUBMISSIONS, "submission_v4_nn.csv"), index=False)
print(f"submission_v4_nn.csv saved ({len(sub)} rows)")
print(preds)
