"""
V4: Feature Engineering + Better LightGBM + Stacking Ensemble
Real columns: sleep_duration, heart_rate, bmi, calorie_expenditure, step_count,
...
def add_features(df):
    ...
    # Encode categoricals numerically for interaction features
    ...
    # Combined risk/health scores
    risk_cols = []
    for c in ["stress_num", "quality_num", "smoking_num"]:
        if c in df.columns:
            risk_cols.append(c)
    if risk_cols:
    ...
    health_cols = []
    for c in ["activity_num", "quality_num"]:
        if c in df.columns:
...
def load_data():
    ...
    y = train[TARGET]
    train_feat = train.drop(columns=[ID_COL, TARGET])
...
def main():
    ...
    print(f"Processed features: {X_tr_proc.shape[1]}")

    results = {}

    # ── 1. LightGBM v2 (regularized) ──
    print("\n" + "=" * 60)
    print("MODEL 1: LIGHTGBM V2 (Regularized)")
    print("=" * 60)
    t0 = time.time()
    ...
    lgb_v2.fit(X_tr_proc, y_tr_enc,
               eval_set=[(X_val_proc, y_val_enc)],
               callbacks=[lgb.early_stopping(200, verbose=True), lgb.log_evaluation(500)])
    ba = balanced_accuracy_score(y_val, le.inverse_transform(lgb_v2.predict(X_val_proc)))
    print(f"Balanced Accuracy: {ba:.4f}")
    print(classification_report(y_val, le.inverse_transform(lgb_v2.predict(X_val_proc))))
    results["LightGBM_v2"] = ba
    print(f"Time: {time.time()-t0:.0f}s")

    # ── 2. LightGBM v3 (conservative) ──
    print("\n" + "=" * 60)
    print("MODEL 2: LIGHTGBM V3 (Conservative)")
    print("=" * 60)
    t0 = time.time()
    ...
    lgb_v3.fit(X_tr_proc, y_tr_enc,
               eval_set=[(X_val_proc, y_val_enc)],
               callbacks=[lgb.early_stopping(300, verbose=True), lgb.log_evaluation(500)])
    ba = balanced_accuracy_score(y_val, le.inverse_transform(lgb_v3.predict(X_val_proc)))
    print(f"Balanced Accuracy: {ba:.4f}")
    print(classification_report(y_val, le.inverse_transform(lgb_v3.predict(X_val_proc))))
    results["LightGBM_v3"] = ba
    print(f"Time: {time.time()-t0:.0f}s")

    # ── 3. Random Forest v4 ──
    print("\n" + "=" * 60)
    print("MODEL 3: RANDOM FOREST V4")
    print("=" * 60)
    t0 = time.time()
    ...
    rf_v4.fit(X_tr_proc, y_tr)
    ba = balanced_accuracy_score(y_val, rf_v4.predict(X_val_proc))
    print(f"Balanced Accuracy: {ba:.4f}")
    print(classification_report(y_val, rf_v4.predict(X_val_proc)))
    results["RandomForest_v4"] = ba
    print(f"Time: {time.time()-t0:.0f}s")

    # ── 4. XGBoost v4 (regularized) ──
    print("\n" + "=" * 60)
    print("MODEL 4: XGBOOST V4 (Regularized)")
    print("=" * 60)
    t0 = time.time()
    ...
    xgb_v4.fit(X_tr_proc, y_tr_enc,
               eval_set=[(X_val_proc, y_val_enc)], verbose=500)
    ba = balanced_accuracy_score(y_val, le.inverse_transform(xgb_v4.predict(X_val_proc)))
    print(f"Balanced Accuracy: {ba:.4f}")
    print(classification_report(y_val, le.inverse_transform(xgb_v4.predict(X_val_proc))))
    results["XGBoost_v4"] = ba
    print(f"Time: {time.time()-t0:.0f}s")

    # ── Summary ──
    print("\n" + "=" * 60)
    print("ALL RESULTS (Validation)")
    print("=" * 60)
    for name, score in sorted(results.items(), key=lambda x: -x[1]):
        print(f"  {name:25s}: {score:.4f}")

    # ── Retrain all on full data + generate submissions ──
    print("\n" + "=" * 60)
    print("RETRAIN ON FULL DATA + SUBMISSIONS")
    print("=" * 60)
    ...
    ba = balanced_accuracy_score(y_val, stack.predict(X_val_proc))
    print(f"Stacking Val Accuracy: {ba:.4f}")
    print(classification_report(y_val, stack.predict(X_val_proc)))
    results["Stacking"] = ba
    print(f"Time: {time.time()-t0:.0f}s")
    ...
    pd.DataFrame({"id": ids, "health_condition": p}).to_csv(os.path.join(SUBMISSIONS, "submission_v4_stacking.csv"), index=False)
    print(f"  submission_v4_stacking.csv")
    print(f"  {p}")
